  ...
  class SpeechQueue {
    ...
    append(text, final = false) {
      ...
      while (this.buffer) {
        ...
        const count = end >= 0 ? end + 1 : this.buffer.length >= 240 ? 240 : final ? this.buffer.length : 0;
        if (!count) break;
        const sentence = this.buffer.slice(0, count).replace(/https?:\/\/\S+/g, '').replace(/\[S\d+\]/g, '').replace(/[*#`]/g, '').trim();
        this.buffer = this.buffer.slice(count);
        if (/[\p{L}\p{N}]/u.test(sentence)) this.queue.push({ text: sentence, context: this.context, segment_id: globalThis.crypto.randomUUID().replaceAll('-', '') });
      }
      void this.drain();
    }
    async drain() {
