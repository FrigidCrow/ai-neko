...
5. 查资料时选择“查攻略”，搜索服务凭据只配置一次。人格、图片和资料会交给同一LangGraph处理；仍由用户操作游戏。
...
- Python全套：457通过、1项非Windows凭据库跳过；人格/记忆检索相关49项。来源为当时工作区运行，后续小改的定向回归在WORKLOG续记。
- 宿主：28项通过。实际Electron原桌宠基线9项及新增闭环9项通过，报告和图片位于[证据目录](evidence/companion)。新增闭环核对人格、记忆CRUD/原文、权限拒绝、所选合成窗口、fake麦克风、真实WebAudio MP3解码/播放、口型、停止和关闭观察。
...
- CI增加实际Windows ZIP的新增闭环门禁：`node desktop/tests/companion.smoke.cjs --archive <zip> --output <json>`，使用包内应用/后端和合成服务；本轮Windows执行结果仍为Pending。Mac源码运行不能替代该门禁，更不能替代Windows11真机。
