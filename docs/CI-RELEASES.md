...
本页记录已发布的 [v0.3.0-alpha.1](https://github.com/FrigidCrow/ai-neko/releases/tag/v0.3.0-alpha.1) [MVP1 桌宠](MVP1-DESKTOP-PET.md) Windows 包与流水线。当前代码已实现 Electron 宿主、透明白裙 YUI Lolita 猫娘、伴随文字聊天和查询来源。**Windows ZIP 构建、实际打包桌面检查和发布 CI 已通过**；Windows 11 真机与真实服务验收仍需另行完成。
...
- 版本下载：[v0.3.0-alpha.1](https://github.com/FrigidCrow/ai-neko/releases/tag/v0.3.0-alpha.1)。选择 `ai-neko-0.3.0-alpha.1-windows-x64.zip`，不是 GitHub 自动生成的 Source code。
...
本轮打包未接入代码签名。发布流水线附带 `build-info.json`、`SHA256SUMS.txt`、`package-smoke.json`、`desktop-smoke.json` 和源码测试报告。可在 PowerShell 执行 `Get-FileHash .\ai-neko-0.3.0-alpha.1-windows-x64.zip -Algorithm SHA256` 与清单比较。YUI 来源与逐文件哈希、N.E.K.O. 通知和 Live2D SDK 独立许可随包提供，见 [资源记录](MVP1-ASSETS.md)。
...
tag 格式为 `v<pyproject 版本>` 加可选预发布后缀，本轮已发布 `v0.3.0-alpha.1`。基础版本必须与 `pyproject.toml` 一致；构建清单记录 `app_version`、`release_version`、源码 commit、Python/Node 锁文件、Electron/后端摘要及素材 manifest。同一版本不可覆盖，已有 Release 会导致发布步骤失败；修复使用新 tag。稳定发布和签名尚未接入。

维护者后续发布时，在已检查的目标 commit 上创建并推送新的版本 tag，由流水线构建和发布；不要重复推送或覆盖已发布的 `v0.3.0-alpha.1`。下载用户直接使用 Release 中的 ZIP。
...
本轮 [发布 CI 运行 36175618386](https://github.com/FrigidCrow/ai-neko/actions/runs/36175618386) 全部成功，构建源 commit 为 `023bee38f29d385bdfc690e2b5c4ea00d4ee5ecb`：Windows Python **302 passed / 0 skipped**，Linux **301 passed / 1 skipped**，桌面宿主 **15 项通过**，冻结后端 **16/16**，实际打包 Electron 桌面 **9/9**，**79 个资源/许可文件核验通过**。Linux 的一个 Windows 凭据用例跳过，不计通过。实际桌面检查包含 YUI 窗口、强制结束 GUI 后清理及重启无重放；`v0.3.0-alpha.1` 已发布。

Windows CI 已通过，但干净 Windows 11 真机验收仍为 Pending：runner 是装有开发工具的 Windows Server。需另外检查 Windows 11 的透明窗口、角色拖动/缩放、托盘、多屏、凭据重开、原版共存、独立数据根和退出清理。真实模型对话和真实搜索验收目前均为 **0**；连续 10 轮真实对话、至少 3 次真实搜索/正文/带来源回答及其质量审核、p95 性能验收仍待完成，不计入合成 CI PASS。音频与 M2 长期记忆尚未实现。
