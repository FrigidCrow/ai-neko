...
**图册保留 P1 设计基线，当前实施进度见 PLAN 和 REVIEW。** M0 已进入实现；图册与文档完成不代表 Windows App 已经可用。
...
| tools：工具与插件边界 | [08](#diagram-08-tools) | 预算、授权、结果与副作用防重放 |
...
- 输入：用户需求与已有教材
- 处理：按 M0 → M6 推进；P0/P1 只交付规划和图解。
- 输出：最终目标是独立的 Windows 桌面伴侣。
- 边界：图中顺序是实施依赖，没有虚构具体日期或工期。

复用与学习：规划参考全部 24 课，按阶段选择移植。 对应课程 L01、L09、L23、L24，见 [教程与源码映射](REFERENCES.md)。
...
- 输入：用户当前输入
- 处理：Runtime 登记回合，图检索并调用模型，输出按轮次投递。
- 输出：已确认的对话记录与待整理任务。
- 边界：取消和崩溃走独立收尾；发送片段不能直接算已显示或已听到。
...
- 处理：校验 → 检索与上下文预算 → 模型 → 可选工具循环 → 幂等提交。流式文字经回复节点、任务用途及当前轮过滤后发送。
...
![工具如何安全执行并把结果交回图](diagrams/svg/08-tools.svg)

[查看 SVG 大图](diagrams/svg/08-tools.svg) · [编辑 Mermaid 源文件](diagrams/src/08-tools.mmd)

- 输入：tool_call_id、args、context、operation_id，及已确认的授权范围。
- 处理：工具与参数校验 → 预算和当前轮校验 → 只读执行或持久操作去重 → 记录结果 → 按 call_id 交回图。M1 使用合成只读工具，M5 接至少两个实际只读工具。
- 输出：成功、无结果、参数错误、超时、取消或执行状态不确定等结构化结果；工具文本仅作资料。
- 边界：本地记录不能保证任意外部动作恰好执行一次。外部已完成但本地未记录时，要利用工具的幂等协议或查询结果；没有恢复能力则报告不确定，不能盲目重放。

复用与学习：按需提取纯工具和 MCP 协议适配；旧任务执行器与完整 Agent 只参考约束，不再开启第二个规划循环。插件管理仍为后续选择项。 对应课程 L03、L17、L18、L19、L21，见 [教程与源码映射](REFERENCES.md)。
...
- 处理：图像做大小、格式和归属检查；主动事件遵守静音、忙碌、冷却与开关。Runtime 统一排队和抢占，始终让用户输入优先。
- 输出：由唯一对话图生成的回复；界面可见来源、触发原因和启停状态，所有媒体结果绑定当前回合。
- 边界：关闭视觉后停止采集或发送新图，旧截图、迟到结果和被抢占事件丢弃。模型不支持图片时明确报错；退出要清定时器和后台任务。
...
![复用界面和角色时需要接上哪些线](diagrams/svg/15-ui-avatar.svg)

[查看 SVG 大图](diagrams/svg/15-ui-avatar.svg) · [编辑 Mermaid 源文件](diagrams/src/15-ui-avatar.mmd)

- 输入：用户交互、角色资料和模型资源，以及带角色/会话/turn_id 的文本、情绪、音频与终态事件。
- 处理：适配器转换协议；消费前检查归属并去重。文字更新聊天框，情绪进入渲染器，实际播放时钟和音量驱动口型。
- 输出：可交互的聊天界面、角色表情和与实际声音同步的口型；生成状态和播放状态分别显示。
- 边界：角色切换、重连、取消后拒收旧事件；不重播已确认音频。模型加载失败显示占位与错误，切换/退出释放资源，不能靠重新生成模型回复掩盖加载失败。

复用与学习：计划评估聊天消息组件和一种角色渲染路径，需适配全局回调/API 并逐项检查素材许可；这部分不代表外部 Electron 窗口与托盘已经取得。 对应课程 L12、L13、L14、L06，见 [教程与源码映射](REFERENCES.md)。
...
- 输入：桌面入口与独立应用身份
- 处理：取得宿主 → 启动后端 → 健康检查 → 加载前端 → 管理退出。
- 输出：可操作的窗口、托盘与角色。
- 边界：独立 N.E.K.O.-PC 的访问、许可和接口未验证；Python exe 不等于完整桌面 App。
