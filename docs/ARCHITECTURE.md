...
状态：整体设计基线；M0 基础与 M1 网页聊天/查询后端已实现，见第 9 节及 REVIEW。当前按 [MVP1 桌宠规格](MVP1-DESKTOP-PET.md) 将可见猫娘、文字气泡与桌面宿主作为 M1 必需交付；桌宠宿主与单猫娘已实现，长期事实记忆和媒体尚未实现。产品范围与阶段状态由 [PLAN](PLAN.md) 管理。
