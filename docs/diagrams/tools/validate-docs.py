...
for p in markdown:
    ...
    if len(fences)%2: issues.append('Unclosed fence: '+str(p.relative_to(ROOT)))
    for line in content.splitlines():
        if line.rstrip()!=line: issues.append('Trailing whitespace: '+str(p.relative_to(ROOT)))
    for raw in re.findall(r'\]\(([^\n]*?)\)', content):
        target = raw.strip().strip('<>')
        if urlparse(target).scheme in ('http','https','mailto'): continue
        path_part, _, fragment = target.partition('#')
        if path_part:
            line_match = re.search(r':(\d+)$', path_part)
            if line_match: path_part = path_part[:line_match.start()]
            resolved = Path(unquote(path_part))
            if not resolved.is_absolute(): resolved = p.parent/resolved
        else: resolved = p
        links += 1
        if not resolved.exists(): issues.append(f'Missing link {p.relative_to(ROOT)}: {target}')
            ...
            for heading in re.findall(r'^#{1,6}\s+(.+?)\s*$',data,re.M):
                ...
                number = counts.get(base,0);counts[base]=number+1
                anchors.add(base+('-'+str(number) if number else ''))
            if unquote(fragment) not in anchors: issues.append(f'Missing anchor {p.relative_to(ROOT)}: {target}')
manifest = json.loads((ROOT/'docs/reference-manifest.json').read_text())
for item in manifest['files']:
    p=Path(item['absolutePath'])
    if not p.exists() or sha(p.read_bytes())!=item['sha256']: issues.append('Source hash changed: '+str(p))
...
if source_fingerprint != initial['sourceFingerprint']: issues.append('Source repository status/diff changed from initial baseline')
old = ROOT.parent/'langgraph-companion'
if old.exists(): issues.append('Old project directory still exists')
if git('rev-parse','--show-toplevel').decode().strip()!=str(ROOT): issues.append('Git root mismatch')
...
for p in files:
    ...
    if rel in allowed_old or p==Path(__file__).resolve() or p.suffix not in ('.md','.json','.html','.css','.mjs','.mmd'): continue
    if old_pattern.search(p.read_text()): issues.append('Stale current name: '+rel)
product_stages={}
for name in ['docs/PLAN.md','REVIEW.md']:
    text=(ROOT/name).read_text(); product_stages[name]={}
    for i in range(7):
...
for p in files:
    if p.suffix in ('.md','.mmd','.mjs','.css','.html','.py'):
        ...
        if run.returncode not in (0,1) or run.stdout.strip() or run.stderr.strip(): issues.append('Whitespace diff: '+str(p.relative_to(ROOT))+' '+run.stdout+run.stderr)
...
for item in render['diagrams']:
    for kind,key in [('src','sourceSha256'),('svg','svgSha256')]:
        ...
        if sha(p.read_bytes())!=item[key]: issues.append('Render hash mismatch: '+str(p.relative_to(ROOT)))
if len(render['diagrams'])!=20 or browser['status']!='PASS': issues.append('Diagram validation incomplete')
