...
source_fingerprint = {
    "status_sha256": sha(git("status", "--porcelain=v1", "-z", cwd=SOURCE)),
    "tracked_diff_sha256": sha(git("diff", cwd=SOURCE)),
}
...
allowed_remote_urls = {
    "git@github.com:FrigidCrow/ai-neko.git",
    "https://github.com/FrigidCrow/ai-neko.git",
}
remotes = []
...
old_pattern = re.compile(
    r"langgraph-companion|LangGraph Companion|LangGraphCompanion|LGC_DATA_DIR|langgraph_companion"
)
for p in files:
    ...
    if (
        ...
        or p.suffix not in (".md", ".json", ".html", ".css", ".mjs", ".mmd")
    ):
        continue
...
for p in files:
    if p.suffix in (".md", ".mmd", ".mjs", ".css", ".html", ".py"):
        run = subprocess.run(
            ["git", "diff", "--no-index", "--check", "--", "/dev/null", str(p)],
            capture_output=True,
            text=True,
        )
        if run.returncode not in (0, 1) or run.stdout.strip() or run.stderr.strip():
            issues.append(
                "Whitespace diff: " + str(p.relative_to(ROOT)) + " " + run.stdout + run.stderr
            )
...
report = {
    ...
    "scope": "Rename, documentation, diagrams, reference integrity; no ai-neko product runtime or Windows validation",
    "environment": {
    ...
    "remotes": remotes,
    "oldDirectoryAbsent": not old.exists(),
    "oldNamesAllowedOnlyInHistory": list(sorted(allowed_old)),
    "markdownFiles": len(markdown),
    "localLinks": links,
    "referenceFiles": len(manifest["files"]),
    "courses": len(manifest["courses"]),
    "referenceGroups": len(manifest["groups"]),
    "sourceHead": git("rev-parse", "HEAD", cwd=SOURCE).decode().strip(),
    "sourceFingerprintCommands": ["git status --porcelain=v1 -z", "git diff"],
    "sourceFingerprint": source_fingerprint,
    "sourceTrackedDiffAndStatusUnchanged": source_fingerprint == initial["sourceFingerprint"],
    "diagramCount": len(render["diagrams"]),
